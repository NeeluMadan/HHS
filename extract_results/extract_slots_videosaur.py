...
# ── Edit these ────────────────────────────────────────────────────────────────
CHECKPOINT_PATH = "./checkpoints/videosaur-ytvis.ckpt"
CONFIG_PATH     = "./baselines/videosaur/configs/videosaur/ytvis.yml"
VIDEOSAUR_ROOT  = "./baselines/videosaur"

VAL_SHARDS  = "/root/datasets/AICF/ytvis2021_resized/ytvis-validation-{000000..000029}.tar"
OUTPUT_DIR  = Path("/raid/create.nm43gr/hyperbolic_slots/data_1/slot_extractions_videosaur_dinov1")
...
def patch_initializer(model, n_slots):
...
def extract_slots(model, video_tensor, n_slots, device):
    patch_initializer(model, n_slots)
...
def main():
    ...
    for n in SLOT_LEVELS:
        patch_initializer(model, n)
        print(f"  n={n:2d} | n_slots={model.initializer.n_slots}")
    patch_initializer(model, SLOT_LEVELS[0])
    print("\nPatch OK — starting extraction\n")
